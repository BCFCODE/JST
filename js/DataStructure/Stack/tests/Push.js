...
export const beforePush = (myStack) => {
  ...
  const toBePaths = ["top.value"];

  const toBeNullPaths = ["top.next"];
...
export const validatePushOperations = ({ myStack, pushValue }) => {
  ...
  const toEqualPaths = ["top.next", "top.next.next", "top.next.next.next"];

  const toBeNullPaths = ["top.next.next.next.next"];
